...
// // class가 search로 박스를 클릭했을 경우 이벤트가 발생
// searchEl.addEventListener('click', function() {
//     // searchInputEl에서 포커스 이벤트 발생
//     searchInputEl.focus();
// });

// // tag가 inpit박스인 것에 focus 이벤트 등록
// searchInputEl.addEventListener('focus', function() {
//     searchEl.classList.add('focused');
//     searchInputEl.setAttribute('placeholder', '통합검색');
// });

// // tag가 inpit박스인 것에 blur 이벤트 등록
...
window.addEventListener('scroll', _.throttle(() => {
    ...
    if (window.scrollY > 500) {
        ...
        gsap.to(badgeEl, .5, {opacity: 0, displau: 'none'})
    } else {
        //배지 보이기
        // badgeEl.style.display = 'block';
        gsap.to(badgeEl, .5, {opacity: 1, displau: 'block'})
    }
}, 500));   // 500ms마다 이벤트 발생
...
fadeEls.forEach(function(fadeEl, index) {
    gsap.to(fadeEl, 1, {
        // dalay값은 몇 초뒤에 실행할지 결정
        delay: (index +1) * .7,     // 0.7, 1.4, 2.1, 2.8dsq    ac
        opacity: 1
    });
});
...
const swiperNotice = new Swiper('.notice-line .swiper-container', {
    ...
    autoplay: {
        delay:1000,
        disableOnInteraction: false,  // 1초마다 자동재생
    }, // 자동 재생 여부
  });
...
const promotionElToggleBtn = document.querySelector('.toggle-promotion')
let isHidePromotion = false;
promotionElToggleBtn.addEventListener('click', function(e){
...
window.addEventListener('scroll', _.throttle(() => {
    ...
    if (window.scrollY > 500) {
        // 배지 보이기
        // badgeEl.style.display = 'none';
        gsap.to(toTopEl, .5, {opacity: 1, displau: 'block'})
    } else {
        //배지 안보이기
        // badgeEl.style.display = 'block';
        gsap.to(toTopEl, .5, {opacity: 0, displau: 'none'})
    }
}, 500)); 
